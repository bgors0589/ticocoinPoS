...
static const int COINBASE_MATURITY_TGC = 500;
...
static const uint256 hashGenesisBlockOfficial("0x000000088e5d0ac927ba694a3ed17b4688c2783cf3b5a6c1afd7d28af9ac636a");
static const uint256 hashGenesisBlockTestNet("0x000000092b2f11197805a98f57b500493a56eb02337101a237eda54024919344");
...
public:
    ...
    {
        // tgccoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // tgccoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // tgccoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // tgccoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // tgccoin: whether transaction is a coinstake
    bool fCoinStake;

    // tgccoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // tgccoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // tgccoin
            undo.nTime = nTime;            // tgccoin
        }
        return true;
    }
...
public:
    ...
    // tgccoin: block signature - signed by coin base txout[0]'s owner
    ...
    // tgccoin: two types of block: proof-of-work or proof-of-stake
    ...
    // tgccoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // tgccoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // tgccoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (tgccoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // tgccoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // tgccoin: proof-of-stake related block index fields
    unsigned int nFlags;  // tgccoin: block index flags
    enum
    {
