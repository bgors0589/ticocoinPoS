...
// tgccoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //tgccoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// tgccoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // tgccoin: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            ploop
            {
                ...
                // tgccoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // tgccoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// tgccoin: create coin stake transaction
...
// tgccoin: check 'spent' consistency between wallet and txindex
// tgccoin: fix wallet spent state according to txindex
...
// tgccoin: disable transaction (only for coinstake)
