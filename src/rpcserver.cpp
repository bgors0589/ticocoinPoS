...
// tgccoin: remove transaction from mempool
...
// tgccoin: reserve balance from being staked for network protection
...
// tgccoin: check wallet integrity
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Ticocoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Ticocoin server stopping";
}
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use ticocoind";
